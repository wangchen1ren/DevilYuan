...
# For Python3.6, remove problem for PyQt5 conflicting with pywinauto
...
class YHClientTrader(ClientTrader):
    ...
    def _handle_verify_code(self):
        ...
        file_path = tempfile.mktemp(dir=self.__tempDir)
    ...
    def balance(self):
        ...
        return self._get_grid_data(self._config.BALANCE_GRID_CONTROL_ID, '资金')
    ...
    def _get_grid_data(self, control_id, name=None):
        i = 1
        while True:
            ...
            try:
                df = self._get_grid_data_inner(control_id, i)
            except:
                print('银河证券客户端: 从剪贴板获取[{}]数据失败'.format(name))
            else:
                break
    ...
    def _get_grid_data_inner(self, control_id, wait=0):
    ...
    def _format_grid_data(self, data):
        ...
        df = pd.read_csv(reader, sep='\t', dtype=str)
